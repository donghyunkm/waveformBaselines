#!/usr/bin/env python3
from __future__ import annotations

import argparse
import csv
import json
import re
import sys
from pathlib import Path
from typing import Any

import numpy as np

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

SUMMARY_JSON = "bland_altman_full_features_summary.json"
DEFAULT_OUTPUT_DIR = Path("blandaltman_full_features")
DEFAULT_DOC = Path("docs/full_data/extractedFeaturesRegressionFullData.md")
DEFAULT_BOOTSTRAP_REPLICATES = 2000
DEFAULT_SEED = 42

# No clinically meaningful Bland-Altman tolerances were found in repository docs,
# task specs, tests, or configs on 2026-09-01. Keep this explicit so coverage is
# not silently computed from arbitrary thresholds.
BLAND_ALTMAN_TOLERANCES: dict[str, float | None] = {
    "ABP_area_ABP_tau_t_plus_0m_gap": None,
    "ABP_area_ShockIdx_t_plus_0m_gap": None,
    "ABP_area_t_plus_0m_gap": None,
    "ABP_tau_t_plus_0m_gap": None,
    "DBP_t_plus_0m_gap": None,
    "ECG_Ramp_t_plus_0m_gap": None,
    "HRV_RMSSD_t_plus_0m_gap": None,
    "HR_range_t_plus_0m_gap": None,
    "HR_t_plus_0m_gap": None,
    "MAP_t_plus_0m_gap": None,
    "PLETH_ACDC_ABP_tau_t_plus_0m_gap": None,
    "PLETH_ACDC_PLETH_amp_t_plus_0m_gap": None,
    "PLETH_ACDC_ShockIdx_t_plus_0m_gap": None,
    "PLETH_ACDC_t_plus_0m_gap": None,
    "PLETH_amp_ShockIdx_t_plus_0m_gap": None,
    "PLETH_amp_t_plus_0m_gap": None,
    "PPV_t_plus_0m_gap": None,
    "PP_t_plus_0m_gap": None,
    "PTT_t_plus_0m_gap": None,
    "PVI_t_plus_0m_gap": None,
    "RESP_amp_t_plus_0m_gap": None,
    "RR_t_plus_0m_gap": None,
    "SBP_t_plus_0m_gap": None,
    "ShockIdx_ABP_tau_t_plus_0m_gap": None,
    "ShockIdx_t_plus_0m_gap": None,
    "dPdt_max_t_plus_0m_gap": None,
}


TRAIN_MEAN_CACHE: dict[tuple[str, str], dict[str, float]] = {}


def finite_or_none(value: float) -> float | None:
    return float(value) if np.isfinite(value) else None


def format_num(value: float | None, decimals: int = 4) -> str:
    if value is None or not np.isfinite(value):
        return "NA"
    return f"{value:.{decimals}f}"


def format_estimate(row: dict[str, Any], key: str, decimals: int = 4) -> str:
    value = row.get(key)
    lower = row.get(f"{key}_lower")
    upper = row.get(f"{key}_upper")
    if value is None or lower is None or upper is None:
        return "NA"
    return f"{format_num(value, decimals)} [{format_num(lower, decimals)}, {format_num(upper, decimals)}]"


def format_pct(row: dict[str, Any], key: str) -> str:
    value = row.get(key)
    lower = row.get(f"{key}_lower")
    upper = row.get(f"{key}_upper")
    if value is None or lower is None or upper is None:
        return "NA"
    return f"{100.0 * value:.1f}% [{100.0 * lower:.1f}%, {100.0 * upper:.1f}%]"


def load_existing_analyses(output_dir: Path) -> list[dict[str, Any]]:
    summary_path = output_dir / SUMMARY_JSON
    analyses = json.loads(summary_path.read_text())
    if not isinstance(analyses, list):
        raise ValueError(f"{summary_path} must contain a list")
    for row in analyses:
        for key in ("target", "best_model", "prediction_file", "plot_path", "stats_path"):
            if key not in row:
                raise ValueError(f"{summary_path} row missing {key}: {row}")
    return sorted(analyses, key=lambda row: row["target"])


def load_prediction_data(prediction_file: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    with np.load(prediction_file, allow_pickle=True) as data:
        for key in ("predictions", "targets", "patient_ids"):
            if key not in data.files:
                raise ValueError(f"{prediction_file} missing {key}")
        predictions = np.asarray(data["predictions"], dtype=np.float64)
        targets = np.asarray(data["targets"], dtype=np.float64)
        patient_ids = np.asarray(data["patient_ids"]).astype(str)
        masks = np.asarray(data["masks"], dtype=bool) if "masks" in data.files else np.ones(targets.shape[0], dtype=bool)
    valid = masks & np.isfinite(predictions) & np.isfinite(targets)
    return predictions[valid], targets[valid], patient_ids[valid]


def bland_altman_stats(predictions: np.ndarray, targets: np.ndarray, tolerance: float | None) -> dict[str, float | None]:
    difference = predictions - targets
    average = 0.5 * (predictions + targets)
    n = int(difference.size)
    if n < 2:
        raise ValueError("Bland-Altman stats require at least two rows")
    bias = float(np.mean(difference))
    diff_std = float(np.std(difference, ddof=1))
    loa_half_width = float(1.96 * diff_std)
    x_centered = average - float(np.mean(average))
    denom = float(np.sum(x_centered * x_centered))
    slope = float(np.sum(x_centered * (difference - bias)) / denom) if denom > np.finfo(float).eps else np.nan
    coverage = None if tolerance is None else float(np.mean(np.abs(difference) <= tolerance))
    return {
        "bias": bias,
        "difference_std": diff_std,
        "loa_half_width": loa_half_width,
        "lower_loa": bias - loa_half_width,
        "upper_loa": bias + loa_half_width,
        "proportional_bias_slope": slope,
        "coverage_probability": coverage,
    }


def patient_sufficient_stats(predictions: np.ndarray, targets: np.ndarray, patient_ids: np.ndarray, tolerance: float | None) -> dict[str, np.ndarray]:
    unique, inverse = np.unique(patient_ids.astype(str), return_inverse=True)
    diff = predictions - targets
    avg = 0.5 * (predictions + targets)
    stats = {
        "patients": unique,
        "n": np.bincount(inverse).astype(np.float64),
        "diff_sum": np.bincount(inverse, weights=diff).astype(np.float64),
        "diff_sq_sum": np.bincount(inverse, weights=diff * diff).astype(np.float64),
        "avg_sum": np.bincount(inverse, weights=avg).astype(np.float64),
        "avg_sq_sum": np.bincount(inverse, weights=avg * avg).astype(np.float64),
        "avg_diff_sum": np.bincount(inverse, weights=avg * diff).astype(np.float64),
    }
    if tolerance is not None:
        stats["coverage_count"] = np.bincount(inverse, weights=(np.abs(diff) <= tolerance).astype(np.float64)).astype(np.float64)
    return stats


def stats_from_patient_counts(stats: dict[str, np.ndarray], counts: np.ndarray, tolerance: float | None) -> dict[str, np.ndarray]:
    n = counts @ stats["n"]
    diff_sum = counts @ stats["diff_sum"]
    diff_sq_sum = counts @ stats["diff_sq_sum"]
    avg_sum = counts @ stats["avg_sum"]
    avg_sq_sum = counts @ stats["avg_sq_sum"]
    avg_diff_sum = counts @ stats["avg_diff_sum"]
    bias = diff_sum / n
    ss_diff = diff_sq_sum - (diff_sum * diff_sum) / n
    diff_std = np.sqrt(np.maximum(ss_diff / np.maximum(n - 1.0, 1.0), 0.0))
    loa_half_width = 1.96 * diff_std
    ss_avg = avg_sq_sum - (avg_sum * avg_sum) / n
    cov = avg_diff_sum - (avg_sum * diff_sum) / n
    slope = np.full(counts.shape[0], np.nan, dtype=np.float64)
    valid = ss_avg > np.finfo(float).eps
    slope[valid] = cov[valid] / ss_avg[valid]
    coverage = None
    if tolerance is not None:
        coverage = (counts @ stats["coverage_count"]) / n
    return {"bias": bias, "loa_half_width": loa_half_width, "proportional_bias_slope": slope, "coverage_probability": coverage}


def bootstrap_cis(predictions: np.ndarray, targets: np.ndarray, patient_ids: np.ndarray, tolerance: float | None, n_bootstrap: int, seed: int) -> dict[str, Any]:
    grouped = patient_sufficient_stats(predictions, targets, patient_ids, tolerance)
    n_patients = int(grouped["patients"].size)
    rng = np.random.default_rng(seed)
    sampled = rng.integers(0, n_patients, size=(n_bootstrap, n_patients))
    counts = np.zeros((n_bootstrap, n_patients), dtype=np.float64)
    for idx, row in enumerate(sampled):
        counts[idx] = np.bincount(row, minlength=n_patients)
    values = stats_from_patient_counts(grouped, counts, tolerance)
    out: dict[str, Any] = {}
    for key in ("bias", "loa_half_width", "proportional_bias_slope"):
        valid = values[key][np.isfinite(values[key])]
        out[f"{key}_lower"] = float(np.percentile(valid, 2.5))
        out[f"{key}_upper"] = float(np.percentile(valid, 97.5))
        out[f"{key}_valid_bootstrap_replicates"] = int(valid.size)
        out[f"{key}_invalid_bootstrap_replicates"] = int(n_bootstrap - valid.size)
    if tolerance is not None and values["coverage_probability"] is not None:
        out["coverage_probability_lower"] = float(np.percentile(values["coverage_probability"], 2.5))
        out["coverage_probability_upper"] = float(np.percentile(values["coverage_probability"], 97.5))
    else:
        out["coverage_probability_lower"] = None
        out["coverage_probability_upper"] = None
    return out



def best_possible_row() -> dict[str, Any]:
    return {
        "target": "ALL TARGETS",
        "model": "PERFECT PREDICTION",
        "row_type": "theoretical_best",
        "tolerance": None,
        "n_predictions": None,
        "n_patients": None,
        "prediction_file": None,
        "plot_path": None,
        "bootstrap_replicates": None,
        "bias": 0.0,
        "bias_lower": None,
        "bias_upper": None,
        "loa_half_width": 0.0,
        "loa_half_width_lower": None,
        "loa_half_width_upper": None,
        "lower_loa": 0.0,
        "upper_loa": 0.0,
        "proportional_bias_slope": 0.0,
        "proportional_bias_slope_lower": None,
        "proportional_bias_slope_upper": None,
        "coverage_probability": 1.0,
        "coverage_probability_lower": None,
        "coverage_probability_upper": None,
    }


def _target_metadata(target_path: Path) -> dict[str, Any]:
    metadata_path = target_path.with_suffix(".json")
    if not metadata_path.exists():
        raise FileNotFoundError(f"Missing target metadata JSON beside {target_path}")
    return json.loads(metadata_path.read_text())


def training_means_original_units(target_path: Path, cache_dir: Path) -> dict[str, float]:
    cache_key = (str(target_path), str(cache_dir))
    if cache_key in TRAIN_MEAN_CACHE:
        return TRAIN_MEAN_CACHE[cache_key]
    metadata = _target_metadata(target_path)
    target_names = list(metadata.get("feature_target_names", []))
    if not target_names:
        raise ValueError(f"No feature_target_names found in {target_path.with_suffix('.json')}")
    with np.load(target_path, allow_pickle=True) as target_data:
        targets = np.asarray(target_data["feature_targets"], dtype=np.float64)
        target_mask = np.asarray(target_data["feature_mask"], dtype=bool)
        target_anchor_ids = np.asarray(target_data["anchor_ids"], dtype=np.int64)
    cache_anchor_ids = np.asarray(np.load(cache_dir / "anchor_ids.npy", allow_pickle=True), dtype=np.int64)
    split_labels = np.asarray(np.load(cache_dir / "split_labels.npy", allow_pickle=True)).astype(str)
    if cache_anchor_ids.shape != split_labels.shape:
        raise ValueError(f"Cache anchor_ids and split_labels shape mismatch in {cache_dir}")
    if np.array_equal(target_anchor_ids, cache_anchor_ids):
        train_rows = split_labels == "train"
    else:
        split_by_anchor = {int(anchor_id): split for anchor_id, split in zip(cache_anchor_ids.tolist(), split_labels.tolist())}
        train_rows = np.asarray([split_by_anchor.get(int(anchor_id)) == "train" for anchor_id in target_anchor_ids.tolist()], dtype=bool)
        if int(train_rows.sum()) == 0:
            raise ValueError(f"No train rows aligned between {target_path} and {cache_dir}")
    means: dict[str, float] = {}
    for col, target in enumerate(target_names):
        valid = train_rows & target_mask[:, col] & np.isfinite(targets[:, col])
        if not np.any(valid):
            raise ValueError(f"No valid training targets for {target}")
        means[target] = float(np.mean(targets[valid, col]))
    TRAIN_MEAN_CACHE[cache_key] = means
    return means


def train_mean_for_analysis(analysis: dict[str, Any]) -> tuple[float, str, str]:
    config_path = Path(analysis["prediction_file"]).parent / "config.json"
    config = json.loads(config_path.read_text())
    target_path = Path(config["target_path"])
    cache_dir = Path(config["cache_dir"])
    means = training_means_original_units(target_path, cache_dir)
    target = analysis["target"]
    if target not in means:
        raise ValueError(f"Target {target!r} not found in training means from {target_path}")
    return means[target], str(target_path), str(cache_dir)


def null_model_row(analysis: dict[str, Any], references: np.ndarray, patient_ids: np.ndarray, tolerance: float | None, n_bootstrap: int, seed: int) -> dict[str, Any]:
    train_mean, target_path, cache_dir = train_mean_for_analysis(analysis)
    null_predictions = np.full(references.shape, train_mean, dtype=np.float64)
    point = bland_altman_stats(null_predictions, references, tolerance)
    boot = bootstrap_cis(null_predictions, references, patient_ids, tolerance, n_bootstrap=n_bootstrap, seed=seed)
    test_mean = float(np.mean(references))
    bias_expected = train_mean - test_mean
    slope_expected = -2.0
    return {
        "target": analysis["target"],
        "model": "TRAIN-MEAN NULL MODEL",
        "row_type": "train_mean_null_model",
        "baseline_type": "train_mean",
        "baseline_prediction_value": train_mean,
        "training_target_mean": train_mean,
        "target_path": target_path,
        "cache_dir": cache_dir,
        "test_reference_mean": test_mean,
        "bias_expected_from_train_minus_test_mean": bias_expected,
        "bias_matches_train_minus_test_mean": bool(abs(float(point["bias"]) - bias_expected) <= 1e-10),
        "null_slope_expected": slope_expected,
        "null_slope_abs_diff_from_minus_two": abs(float(point["proportional_bias_slope"]) - slope_expected),
        "null_slope_sanity_check_passed": bool(abs(float(point["proportional_bias_slope"]) - slope_expected) <= 1e-10),
        "tolerance": tolerance,
        "n_predictions": int(references.size),
        "n_patients": int(np.unique(patient_ids).size),
        "prediction_file": analysis["prediction_file"],
        "plot_path": None,
        "bootstrap_replicates": n_bootstrap,
        **point,
        **boot,
    }


def row_from_analysis(analysis: dict[str, Any], n_bootstrap: int, seed: int) -> tuple[dict[str, Any], dict[str, Any]]:
    target = analysis["target"]
    model = analysis["best_model"]
    tolerance = BLAND_ALTMAN_TOLERANCES.get(target)
    predictions, references, patient_ids = load_prediction_data(Path(analysis["prediction_file"]))
    point = bland_altman_stats(predictions, references, tolerance)
    boot = bootstrap_cis(predictions, references, patient_ids, tolerance, n_bootstrap=n_bootstrap, seed=seed)
    actual = {
        "target": target,
        "model": model,
        "row_type": "model",
        "tolerance": tolerance,
        "n_predictions": int(predictions.size),
        "n_patients": int(np.unique(patient_ids).size),
        "prediction_file": analysis["prediction_file"],
        "plot_path": analysis["plot_path"],
        "bootstrap_replicates": n_bootstrap,
        **point,
        **boot,
    }
    null_row = null_model_row(analysis, references, patient_ids, tolerance, n_bootstrap=n_bootstrap, seed=seed)
    return actual, null_row


def validate_against_existing_plot_stats(rows: list[dict[str, Any]], analyses: list[dict[str, Any]], required_targets: set[str]) -> list[dict[str, Any]]:
    model_rows = {(row["target"], row["model"]): row for row in rows if row["row_type"] == "model"}
    checks = []
    for analysis in analyses:
        target = analysis["target"]
        if target not in required_targets:
            continue
        stats_path = Path(analysis["stats_path"])
        existing = json.loads(stats_path.read_text())
        row = model_rows[(target, analysis["best_model"])]
        bias = float(row["bias"])
        lower = bias - float(row["loa_half_width"])
        upper = bias + float(row["loa_half_width"])
        check = {
            "target": target,
            "model": analysis["best_model"],
            "stats_path": str(stats_path),
            "bias_abs_diff": abs(bias - float(existing["bias"])),
            "lower_loa_abs_diff": abs(lower - float(existing["loa_lower"])),
            "upper_loa_abs_diff": abs(upper - float(existing["loa_upper"])),
        }
        check["matches_existing_plot_stats"] = all(check[key] <= 1e-9 for key in ("bias_abs_diff", "lower_loa_abs_diff", "upper_loa_abs_diff"))
        checks.append(check)
    found = {row["target"] for row in checks}
    missing = required_targets - found
    if missing:
        raise ValueError(f"Missing validation targets: {sorted(missing)}")
    return checks


def markdown_table(rows: list[dict[str, Any]]) -> str:
    lines = [
        "## Bland-Altman Agreement Summary",
        "",
        "This table summarizes the existing full-data best-model Bland-Altman analyses in `blandaltman_full_features/`. Statistics use `difference = prediction - reference` and `average = (prediction + reference) / 2`, matching the plot-generation convention. Model rows use patient-cluster bootstrap percentile 95% CIs with patients resampled as clusters and all windows retained for sampled patients. The `TRAIN-MEAN NULL MODEL` predicts the training-set mean of the corresponding target for every test observation, in the same original physical units as the Bland-Altman plots; brackets on model and null rows are patient-cluster bootstrap percentile 95% CIs.",
        "",
        f"Bootstrap replicates: `{DEFAULT_BOOTSTRAP_REPLICATES}` by default. Random seed: `{DEFAULT_SEED}` by default. No repository-defined clinically meaningful tolerances were found for these regression targets, so `Within Tolerance` is `NA` until `BLAND_ALTMAN_TOLERANCES` in `scripts/summarize_bland_altman_full_features.py` is populated with explicit defensible thresholds.",
        "",
        "**Reference rows.** `PERFECT PREDICTION` denotes exact agreement (`prediction = reference`), giving zero bias, zero LoA width, zero proportional-bias slope, and 100% coverage. `TRAIN-MEAN NULL MODEL` predicts the training-set mean of the corresponding target for every test observation and represents a model with no patient- or window-specific predictive information. The null prediction is derived exclusively from the training data. Lower absolute bias is better, lower LoA half-width is better, proportional-bias slope closer to zero is better, and higher coverage is better. Do not compare raw Bias or LoA half-width across targets with different physical units; compare those primarily within a target.",
        "",
        "Machine-readable outputs: `blandaltman_full_features/bland_altman_agreement_summary.csv` and `blandaltman_full_features/bland_altman_agreement_summary.json`.",
        "",
        "| Target | Model | Bias [95% CI] | LoA Half-Width [95% CI] | Proportional-Bias Slope [95% CI] | Within Tolerance [95% CI] | N Predictions | N Patients |",
        "|---|---|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        if row["row_type"] == "theoretical_best":
            bias = "0"
            loa = "0"
            slope = "0"
            coverage = "100%"
            n_predictions = "-"
            n_patients = "-"
        else:
            bias = format_estimate(row, "bias")
            loa = format_estimate(row, "loa_half_width")
            slope = format_estimate(row, "proportional_bias_slope")
            coverage = format_pct(row, "coverage_probability")
            n_predictions = str(row["n_predictions"])
            n_patients = str(row["n_patients"])
        lines.append(f"| `{row['target']}` | `{row['model']}` | {bias} | {loa} | {slope} | {coverage} | {n_predictions} | {n_patients} |")
    return "\n".join(lines) + "\n"


def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    fields = [
        "target", "model", "row_type", "bias", "bias_lower", "bias_upper", "loa_half_width", "loa_half_width_lower", "loa_half_width_upper",
        "lower_loa", "upper_loa", "proportional_bias_slope", "proportional_bias_slope_lower", "proportional_bias_slope_upper",
        "coverage_probability", "coverage_probability_lower", "coverage_probability_upper", "tolerance", "n_predictions", "n_patients",
        "bootstrap_replicates", "baseline_type", "baseline_prediction_value", "training_target_mean", "test_reference_mean", "bias_expected_from_train_minus_test_mean", "bias_matches_train_minus_test_mean", "null_slope_expected", "null_slope_abs_diff_from_minus_two", "null_slope_sanity_check_passed", "target_path", "cache_dir", "prediction_file", "plot_path",
    ]
    with path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fields, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(rows)


def update_doc(doc_path: Path, section: str) -> None:
    text = doc_path.read_text()
    if "## Bland-Altman Agreement Summary" in text:
        text = re.sub(r"## Bland-Altman Agreement Summary\n.*?(?=\n## |\Z)", section.rstrip() + "\n", text, flags=re.S)
    else:
        marker = "\n## Verification\n"
        if marker not in text:
            raise ValueError(f"Could not find insertion marker in {doc_path}")
        text = text.replace(marker, "\n" + section + marker)
    doc_path.write_text(text)


def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize full-data best-model Bland-Altman analyses with train-mean null rows.")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--doc", type=Path, default=DEFAULT_DOC)
    parser.add_argument("--bootstrap-replicates", type=int, default=DEFAULT_BOOTSTRAP_REPLICATES)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    parser.add_argument("--no-doc-update", action="store_true")
    args = parser.parse_args()

    analyses = load_existing_analyses(args.output_dir)
    rows: list[dict[str, Any]] = [best_possible_row()]
    for analysis in analyses:
        rows.extend(row_from_analysis(analysis, args.bootstrap_replicates, args.seed))
    validation = validate_against_existing_plot_stats(rows, analyses, {"HR_t_plus_0m_gap", "RR_t_plus_0m_gap", "SBP_t_plus_0m_gap"})
    summary_json = args.output_dir / "bland_altman_agreement_summary.json"
    summary_csv = args.output_dir / "bland_altman_agreement_summary.csv"
    summary_md = args.output_dir / "bland_altman_agreement_summary.md"
    payload = {
        "bootstrap_replicates": args.bootstrap_replicates,
        "seed": args.seed,
        "tolerances": BLAND_ALTMAN_TOLERANCES,
        "validation_checks": validation,
        "rows": rows,
    }
    summary_json.write_text(json.dumps(payload, indent=2, sort_keys=True))
    write_csv(summary_csv, rows)
    section = markdown_table(rows)
    summary_md.write_text(section)
    if not args.no_doc_update:
        update_doc(args.doc, section)
    print(json.dumps({
        "analyses_found": len(analyses),
        "rows_written": len(rows),
        "bootstrap_replicates": args.bootstrap_replicates,
        "seed": args.seed,
        "summary_json": str(summary_json),
        "summary_csv": str(summary_csv),
        "summary_md": str(summary_md),
        "validation_checks": validation,
        "targets_without_tolerance": sorted(k for k, v in BLAND_ALTMAN_TOLERANCES.items() if v is None),
    }, indent=2, sort_keys=True))


if __name__ == "__main__":
    main()
